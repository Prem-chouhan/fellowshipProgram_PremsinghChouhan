...
class AddressBook:
    ...
    def __init__(self, firstName=None, lastName=None, addressUser=None, city=None, state=None, zipCode=000000,
...
class AddressFacade(object):
    def __init__(self):
        self.my_address_book = AddressBook()

    def startAddressBook(self):
        while True:
            try:
                while True:
                    ...
                    choice = int(
                        input("Enter what operation you have to perform in Address book:-\n1.Add Record\n2.Edit "
                              "Record\n3.Delete Record\n4.Search Record\n5.Quit Address book"))
                    if choice == 1:
                        self.my_address_book.addRecord()

                    if choice == 2:
                        updaName = input("Enter name Whose record you have to update")
                        self.my_address_book.editRecord(updaName)

                    if choice == 3:
                        delName = input("Enter First name of Person Whose Record is to be deleted:")
                        self.my_address_book.deleteRecord(delName)

                    if choice == 4:
                        searName = input("Enter First name of Person Whose Record is to be Searched:")
                        self.my_address_book.searchRecord(searName)
...
def main():
    my_address_facade = AddressFacade()
    my_address_facade.startAddressBook()
